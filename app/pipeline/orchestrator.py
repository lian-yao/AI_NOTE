...
class PipelineOrchestrator:
    ...
    async def _parse_local_file(self, source_url: str, platform: str) -> StageResult:
        """解析本地文件：创建视频目录，复制文件，生成 meta.json。

        source_url 格式: local://data/uploads/{file_id}.{ext}
        """
        import shutil
        ...
        if local_path.startswith("local://"):
            local_path = local_path.removeprefix("local://")
        ...
        # 生成 video_id（基于文件名 + 时间戳，确保唯一）
        stem = src_path.stem
        # 如果 stem 已经是 uuid 格式（来自上传），直接使用
        if len(stem) >= 8 and all(c in "0123456789abcdef" for c in stem[:8]):
            video_id = f"local_{stem[:16]}"
        else:
            import uuid as _uuid
            video_id = f"local_{_uuid.uuid4().hex[:12]}"

        # 创建视频产物目录
        video_dir = ensure_video_dir(video_id, settings.data_dir)

        # 复制源文件到产物目录
        dest_filename = f"source{suffix}"
        ...
        meta = {
            ...
            "avid": None,
            "platform": "local",
            "local_source_path": str(dest_path),
            "file_size": file_size,
        }
        ...
        return StageResult(
            success=True,
            artifacts={
                "meta_json": str(video_dir / "meta.json"),
    ...
    async def _run_stage(self, task: PipelineTask, stage: PipelineStage, db) -> None:
        ...
        if stage == PipelineStage.PARSE:
            ...
            if is_local:
                ...
                # 保存本地文件路径供后续阶段使用
                local_file_path = parse_result.metadata.get("local_file_path", "")
                task.options["local_file_path"] = local_file_path
            ...
            if self._is_local_source(task):
                ...
                if is_audio and local_path and Path(local_path).is_file():
                    # 音频文件：直接用 ffmpeg 转为 16kHz mono WAV
                    ...
                    # 视频文件：复制到 video_dir 供 extract_audio 使用
                    import shutil
            ...
            # 本地音频文件：DOWNLOAD 阶段已转换为 audio.wav，跳过 extract_audio
            is_local = self._is_local_source(task)
            local_path = task.options.get("local_file_path", "")
            suffix = Path(local_path).suffix.lower() if local_path else ""
