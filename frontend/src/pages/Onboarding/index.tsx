...
function errText(e: unknown): string {
  ...
  if (typeof e === 'object') {
    const item = e as { msg?: unknown; message?: unknown }
...
// 桌面端首启 4 步引导。完成后写 localStorage('ai-video-onboarded') = '1'，路由守卫不再拦。
//
// 1. 后端连通性自检
  ...
  async function saveProvider() {
    ...
    try {
      ...
      try {
        ...
        // 撞名：复用已存在的同名供应商
        const list = ((await getProviderList({ silent: true })) || []) as IProvider[]
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-pink-50 p-6">
      <div className="w-full max-w-xl rounded-xl border bg-white p-6 shadow-lg">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="text-xl font-bold">欢迎使用 AI Video Notes</h1>
        ...
        {step === 1 && (
          <section className="flex flex-col gap-3">
            ...
            {backendOk === true && <div className="rounded bg-primary-light p-2 text-sm text-primary">✓ 后端已就绪</div>}
            {backendOk === false && (
        ...
        {step === 4 && (
          <section className="flex flex-col gap-3">
            ...
            <div className="rounded bg-gray-50 p-3 text-xs text-gray-600">
              提示：浏览器插件支持一键 cookie 同步；桌面版需手动复制。
            </div>
            <div className="flex gap-2 justify-between">
              <button className="text-sm text-gray-500 hover:text-gray-800" onClick={prev}>上一步</button>
