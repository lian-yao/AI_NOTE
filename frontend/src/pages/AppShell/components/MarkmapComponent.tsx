...
import type { IToolbarItem } from 'markmap-toolbar'
...
type CustomToolbarButton = IToolbarItem & { id: string }
...
<head>
  ...
  <title>${title || 'AI Video Notes 思维导图'}</title>
  <style>
  body {
    margin: 0;
    padding: 0;
...
<body>
  ...
  const exportXMind = async () => {
    try {
      ...
      const convertToXMindNode = (node: MarkmapNode, isRoot = false): XMindTopic => {
        const rawTitle = node.content || node.payload?.content || '未命名';
        const xmindNode: XMindTopic = {
          id: generateId(),
      ...
      const rootTopic = convertToXMindNode(root as MarkmapNode, true);
