...
| 项目 | 选择 | 说明 |
|------|------|------|
| 前后端通信 | RESTful API + SSE | 常规请求用 REST，问答流式输出用 SSE |
| 实时进度 | WebSocket / 轮询 | 处理进度实时推送（可降级为轮询） |
| 数据格式 | JSON | 全系统统一 |
...
| 技术 | 版本 | 用途 | 选型理由 |
|------|------|------|---------|
| Vue 3 | ≥ 3.4 | 前端框架 | 组合式 API 简洁，类型友好 |
| Vite | ≥ 5.0 | 构建工具 | 极速 HMR，按需编译 |
| Element Plus | ≥ 2.5 | UI 组件库 | 国内最成熟的中文 UI 库，Vue 3 原生支持 |
| Pinia | ≥ 2.1 | 状态管理 | Vue 3 官方推荐，TypeScript 支持好 |
| vue-router | ≥ 4.2 | 路由 | Vue 3 官方路由 |
| Axios | ≥ 1.6 | HTTP 客户端 | 成熟稳定，拦截器机制完善 |
| marked | ≥ 12 | Markdown 渲染 | 渲染笔记内容，支持代码高亮 |
